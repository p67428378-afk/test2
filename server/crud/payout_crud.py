...
def get_payout_batch(db: Session, batch_id: uuid.UUID):
    return db.query(PayoutBatch).filter(PayoutBatch.id == batch_id).first()

def get_payout_transactions(db: Session, batch_id: uuid.UUID, skip: int = 0, limit: int = 100):
    return db.query(PayoutTransaction).filter(PayoutTransaction.batch_id == batch_id).offset(skip).limit(limit).all()

def count_payout_batches(db: Session):
    return db.query(PayoutBatch).count()

def count_payout_transactions(db: Session, batch_id: uuid.UUID):
    return db.query(PayoutTransaction).filter(PayoutTransaction.batch_id == batch_id).count()
