...
export default function SKUQuadrantChart({
  ...
  return (
    <div className="bg-surface border border-outline-variant rounded-xl shadow-[0px_1px_3px_rgba(0,0,0,0.1),0px_1px_2px_rgba(0,0,0,0.06)] p-4 flex flex-col gap-3">
      ...
      {/* Chart Area */}
      <div className="relative w-full h-64 bg-surface-container-lowest border border-outline-variant rounded-lg overflow-hidden mt-2">
        {/* Quadrant Labels */}
        ...
        <div className="absolute bottom-2 left-2 text-[10px] font-bold text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded z-10">
          Niche / New (Low Space, Low Sales)
        </div>
        <div className="absolute bottom-2 right-2 text-[10px] font-bold text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded z-10">
        ...
        {/* Axis Labels */}
        <div className="absolute left-2 top-1/2 -translate-y-1/2 -rotate-90 origin-left text-[10px] font-bold text-secondary uppercase tracking-wider">
          Sales Velocity (Weekly Sales) &rarr;
        </div>
        <div className="absolute bottom-1 left-1/2 -translate-x-1/2 text-[10px] font-bold text-secondary uppercase tracking-wider">
          Linear Shelf Footprint (ft) &rarr;
        </div>

        {/* Dots */}
        {skus.map((sku) => {
          const xPct = ((sku.linear_shelf_footprint || 0) / maxFootprint) * 100;
          ...
          return (
            <button
              ...
              style={{
                ...
                bottom: `${Math.min(Math.max(yPct, 8), 92)}%`,
              }}
            ...
            >
              ...
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:flex flex-col items-center pointer-events-none z-50">
                <div className="bg-inverse-surface text-inverse-on-surface text-xs rounded py-1 px-2 whitespace-nowrap shadow-md border border-outline-variant">
                  ...
                  <div>Weekly Sales: ${sku.weekly_sales.toLocaleString()}</div>
