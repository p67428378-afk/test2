...
function PanInput() {
  ...
  const [validationStatus, setValidationStatus] = useState('Awaiting document signature');
  ...
  return (
    <section className='col-span-1 bg-surface-container-lowest p-8 rounded-xl shadow-sm space-y-6'>
      ...
      <div className='space-y-4'>
        ...
        <div className={`flex items-center gap-3 bg-${statusColor}/30 p-3 rounded-lg`}>
          <span className={`material-symbols-outlined text-${statusColor}`} data-icon='info' style={{fontVariationSettings: "'FILL' 1"}}>info</span>
          <span className={`text-sm font-semibold text-${statusColor}`}>{validationStatus}</span>
        </div>
      </div>
    </section>
  );
