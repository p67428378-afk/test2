  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setMetrics(mRes);
        setRecentSites(sitesRes.items || []);
        setRecentArtifacts(artRes.items || []);
        setRecentLabAnalyses(labRes.items || []);
      } catch (err) {
      ...
      <div className="bg-gradient-to-r from-amber-900 to-amber-800 text-white p-6 rounded-lg shadow-md flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight font-display">
            Archaeological Excavation Management System
          </h1>
          <p className="text-amber-100 text-sm mt-1">
            Systematic excavation recording, GPS geospatial tracking, artifact
            cataloging, laboratory testing, and academic citations.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-lg border border-stone-200 shadow-sm flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-stone-900">
              {metrics?.active_sites_count ?? recentSites.length}
            </p>
        ...
        <div className="bg-white p-5 rounded-lg border border-stone-200 shadow-sm flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-stone-900">
              {metrics?.cataloged_artifacts_count ?? recentArtifacts.length}
            </p>
            <span className="inline-block px-2 py-0.5 text-[11px] bg-emerald-100 text-emerald-800 rounded-full font-medium mt-0.5">
        ...
        <div className="bg-white p-5 rounded-lg border border-stone-200 shadow-sm flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-stone-900">
              {metrics?.pending_lab_tests_count ?? recentLabAnalyses.length}
            </p>
