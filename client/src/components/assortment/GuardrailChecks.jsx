...
export default function GuardrailChecks({ scenario }) {
  const {
    new_private_brand_percent = 15.4,
  ...
  const guardrails = [
    {
      ...
      description: 'Ensures private brand penetration meets corporate strategic goals.',
    },
    {
      name: 'Shelf Capacity Limit (<95.0%)',
      value: '88.2%',
      status: 'PASSED',
      description: 'Ensures total linear footage utilized does not exceed physical shelf space.',
    },
    {
    ...
    },
    {
      ...
      description: 'Protects category profitability during assortment optimization.',
    },
  ];
          ...
          >
            <div className='flex-1'>
              <div className='flex items-center gap-2'>
                <span className='font-semibold text-on-surface text-sm'>{guardrail.name}</span>
                <span className='text-xs text-primary-fixed-dim font-mono bg-primary-container/10 px-1.5 py-0.5 rounded'>
                  {guardrail.value}
                </span>
              </div>
              <p className='text-xs text-on-surface-variant mt-1'>{guardrail.description}</p>
            </div>
