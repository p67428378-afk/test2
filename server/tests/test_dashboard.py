...
def _seed_dashboard_data():
    ...
    try:
        ...
        battery = EnergySource(
            name="Battery Storage C", type="battery", status="active"
        )
        ...
        metrics = [
            ...
            RealtimeMetric(
                energy_source_id=solar.id,
                metric_name="illumination_lux",
                metric_value=48000.0,
            ),
            RealtimeMetric(
                energy_source_id=solar.id,
                metric_name="daily_energy_kwh",
                metric_value=35.0,
            ),
            RealtimeMetric(
                energy_source_id=wind.id,
                metric_name="power_output_kw",
                metric_value=1.5,
            ),
            RealtimeMetric(
                energy_source_id=wind.id, metric_name="wind_speed_ms", metric_value=9.0
            ),
            RealtimeMetric(
                energy_source_id=wind.id, metric_name="turbine_rpm", metric_value=1300.0
            ),
            RealtimeMetric(
                energy_source_id=battery.id,
                metric_name="charge_level_pct",
                metric_value=90.0,
            ),
            RealtimeMetric(
                energy_source_id=battery.id,
                metric_name="remaining_time_mins",
                metric_value=150.0,
            ),
            RealtimeMetric(
                energy_source_id=battery.id, metric_name="status", metric_value=1.0
            ),
            RealtimeMetric(
                energy_source_id=grid.id, metric_name="power_draw_kw", metric_value=0.8
            ),
            RealtimeMetric(
                energy_source_id=grid.id, metric_name="status", metric_value=1.0
            ),
        ]
        db.add_all(metrics)
        db.commit()
