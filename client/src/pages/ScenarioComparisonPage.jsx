...
export default function ScenarioComparisonPage({ selectedScenario, setSelectedScenario }) {
    ...
    const fetchScenarios = async () => {
      try {
        ...
        if (!selectedScenario && data.length > 0) {
          const balanced = data.find(s => s.name.toLowerCase() === 'balanced') || data[1] || data[0];
    ...
    return (
      <div className='flex-1 flex items-center justify-center min-h-[400px]'>
        <div className='flex flex-col items-center gap-4'>
          ...
          <span className='text-on-surface-variant text-sm'>Loading scenarios...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className='flex flex-col gap-6 w-full'>
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4'>
        <div>
          <h1 className='text-3xl font-bold text-on-surface tracking-tight'>Scenario Comparison</h1>
          <p className='text-sm text-on-surface-variant mt-1'>Compare side-by-side options to optimize Snacks assortment.</p>
        </div>
        <div>
          <button
        ...
        <div className='glass-panel rounded-lg p-6 border border-primary-fixed-dim/30 bg-surface-container-high flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mt-4'>
          <div>
            <div className='flex items-center gap-2'>
              ...
              <h4 className='text-lg font-bold text-on-surface'>{selectedScenario.name} Scenario</h4>
            </div>
