  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const numberItems = allItems.filter((item) => item.type === "number");
        // Sort numerically
        numberItems.sort(
          (a, b) => parseInt(a.value, 10) - parseInt(b.value, 10),
        );

        setItems(numberItems);
        setExploredIds(progress.explored_item_ids || []);
      } catch (err) {
        console.error("Error fetching numbers data:", err);
        ...
        <div className="flex flex-col flex-[6_0_0] items-start min-w-px overflow-clip relative self-stretch">
          <div className="bg-white border border-[#cce0f2] border-solid flex flex-col gap-[16px] items-start overflow-clip p-[16px] relative rounded-[14px] w-full shadow-[0px_2px_4px_0px_rgba(0,0,0,0.08)]">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-[12px] w-full justify-items-center">
