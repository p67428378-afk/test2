...
def get_current_user(db: Session = Depends(get_db)) -> models.User:
    ...
    if not user:
        ...
        user = models.User(
            ...
            is_whole_dollar_catch_all_enabled=False,
        )
        db.add(user)
        db.commit()
        db.refresh(user)
...
def get_roundup_settings(
    ...
    return RoundupSettingsResponse(
        ...
        roundup_multiplier=current_user.roundup_multiplier,
        is_whole_dollar_catch_all_enabled=current_user.is_whole_dollar_catch_all_enabled,
...
def update_roundup_settings(
    ...
    if settings_update.roundup_multiplier < 1:
        raise HTTPException(status_code=422, detail="Multiplier must be at least 1")

    current_user.is_roundup_enabled = settings_update.is_roundup_enabled
    current_user.roundup_multiplier = settings_update.roundup_multiplier
    current_user.is_whole_dollar_catch_all_enabled = (
        settings_update.is_whole_dollar_catch_all_enabled
    )
    db.commit()
