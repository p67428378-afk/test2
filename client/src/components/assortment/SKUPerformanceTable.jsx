  ...
  return (
    <div className='glass-panel rounded-lg flex flex-col border border-[#334155] w-full overflow-hidden'>
      ...
      <div className='p-4 md:p-6 border-b border-[#334155] bg-surface-container-low flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 rounded-t-lg'>
        <div>
          <h2 className='text-xl font-bold text-on-surface'>SKU Performance Table</h2>
          <p className='text-xs text-on-surface-variant mt-1'>Current performance metrics and recommendations for Snacks SKUs.</p>
        </div>
        ...
        <table className='w-full text-left border-collapse min-w-[800px]'>
          <thead>
            <tr className='bg-surface-container-highest border-b border-[#334155]'>
              <th className='p-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider pl-6'>SKU ID</th>
              <th className='p-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider'>Product Description</th>
              <th className='p-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider text-right'>Sales ($)</th>
              <th className='p-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider text-right'>Units Sold</th>
              <th className='p-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider text-right'>Margin %</th>
              <th className='p-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider text-right pr-6'>WOS</th>
              <th className='p-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider text-center'>Recommendation</th>
            </tr>
          </thead>
          <tbody className='font-mono text-sm'>
            {filteredSkus.length === 0 ? (
              <tr>
                <td colSpan='7' className='p-6 text-center text-on-surface-variant font-sans'>
              ...
              filteredSkus.map((sku, index) => (
                ...
                >
                  ...
                  <td className={`p-4 text-right font-bold ${sku.profit_margin >= 30 ? 'text-[#10B981]' : 'text-on-surface'}`}>
                    {sku.profit_margin}%
                  </td>
                  <td className={`p-4 text-right pr-6 ${sku.days_of_supply >= 30 ? 'text-[#F59E0B]' : sku.days_of_supply >= 40 ? 'text-[#EF4444]' : 'text-on-surface'}`}>
                    {sku.days_of_supply}
                  </td>
                  <td className='p-4 text-center'>
                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-bold ${getBadgeStyle(sku.status_badge)}`}>
