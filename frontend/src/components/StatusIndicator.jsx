...
function StatusIndicator() {
  ...
  // This is a placeholder for fetching the status from the backend
  useEffect(() => {
    // api.getKycStatus().then(response => {
    //   setKycStatus(response.data.status);
    // });
    setKycStatus('APPROVED'); // Hardcoded for now
  }, []);
      ...
      <div className='bg-surface-container-lowest rounded-[calc(0.75rem-4px)] p-8 flex flex-col items-center text-center'>
        ...
        <p className='text-sm font-medium text-on-surface-variant mb-8 px-6'>
          {isApproved
            ...
            : 'Your profile has been flagged for review. Please check the audit trail for more details.'}
        </p>
