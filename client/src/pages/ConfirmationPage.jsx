...
export default function ConfirmationPage({ submissionResult, selectedScenario }) {
  ...
  return (
    <div className='flex-1 flex flex-col items-center justify-center min-h-[500px] max-w-2xl mx-auto w-full gap-6'>
      ...
      <div className='text-center space-y-3'>
        ...
        <h1 className='text-3xl font-bold text-on-surface tracking-tight'>Assortment Plan Submitted!</h1>
        <p className='text-sm text-on-surface-variant max-w-md mx-auto'>
          Your Snacks assortment plan for the Small Town Value Cluster has been successfully submitted for regional approval.
        </p>
      </div>
      ...
      <div className='glass-panel rounded-lg border border-[#334155] w-full overflow-hidden'>
        <div className='p-4 md:p-6 border-b border-[#334155] bg-surface-container-low flex items-center gap-2'>
          ...
          <h3 className='text-lg font-bold text-on-surface'>Audit Trail Summary</h3>
        </div>
        <div className='p-6 space-y-4 font-mono text-sm'>
          <div className='flex justify-between items-center border-b border-[#334155]/50 pb-3'>
            <span className='text-xs text-on-surface-variant font-sans font-semibold uppercase tracking-wider'>Submission ID</span>
            <span className='text-on-surface font-bold'>{result.submission_id}</span>
          </div>
          <div className='flex justify-between items-center border-b border-[#334155]/50 pb-3'>
            <span className='text-xs text-on-surface-variant font-sans font-semibold uppercase tracking-wider'>Selected Scenario</span>
            <span className='text-primary-fixed-dim font-bold'>{result.selected_scenario}</span>
          </div>
          <div className='flex justify-between items-center border-b border-[#334155]/50 pb-3'>
            <span className='text-xs text-on-surface-variant font-sans font-semibold uppercase tracking-wider'>Submitted By</span>
            <span className='text-on-surface font-bold'>{result.submitted_by}</span>
          </div>
          <div className='flex justify-between items-center border-b border-[#334155]/50 pb-3'>
            <span className='text-xs text-on-surface-variant font-sans font-semibold uppercase tracking-wider'>Timestamp</span>
            <span className='text-on-surface font-bold'>{new Date(result.timestamp).toLocaleString()}</span>
          </div>
          <div className='flex justify-between items-center pt-1'>
            <span className='text-xs text-on-surface-variant font-sans font-semibold uppercase tracking-wider'>Status</span>
