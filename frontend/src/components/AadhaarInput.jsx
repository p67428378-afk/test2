...
function AadhaarInput() {
  ...
  const [validationStatus, setValidationStatus] = useState('Real-time validation successful');
  const [statusColor, setStatusColor] = useState('tertiary');
      ...
      <div className='space-y-4'>
        ...
        <div className='relative group'>
          ...
          <div className='border-2 border-dashed border-outline-variant bg-surface-container-low rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-surface-container-high transition-colors'>
            ...
            <p className='text-sm font-medium text-on-surface'>aadhaar_front_final.pdf</p>
            <p className='text-xs text-on-surface-variant mt-1'>4.2 MB • Upload Complete</p>
          </div>
        </div>
        <div className={`flex items-center gap-3 bg-${statusColor}/10 p-3 rounded-lg`}>
          <span className={`material-symbols-outlined text-on-${statusColor}-container`} data-icon='check_circle'>check_circle</span>
          <span className={`text-sm font-semibold text-on-${statusColor}-container`}>{validationStatus}</span>
        </div>
      </div>
    </section>
  );
