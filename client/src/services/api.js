...
export const getDashboardKPIs = async () => {
  try {
    ...
    console.warn('Failed to fetch dashboard KPIs from backend, using mock fallback:', error);
    return {
...
export const getSKUPerformance = async () => {
  try {
    ...
    console.warn('Failed to fetch SKU performance from backend, using mock fallback:', error);
    return [
      {
        id: 'sku-1001',
        sku: 'SKU-1001',
...
export const getScenarios = async () => {
  try {
    ...
    console.warn('Failed to fetch scenarios from backend, using mock fallback:', error);
    return [
      {
      ...
      },
      {
        ...
        name: 'Balanced',
        description: 'Moderate optimization balancing private brand growth and shelf space efficiency.',
      ...
      },
      {
        ...
        name: 'Aggressive',
        description: 'High-impact changes maximizing private brand penetration and sales velocity.',
...
export const submitAssortmentPlan = async (scenarioId, userId = 'manager-1') => {
  try {
    ...
    console.warn('Failed to submit assortment plan to backend, using mock fallback:', error);
    return {
      selected_scenario: scenarioId.includes('conservative') ? 'Conservative' : scenarioId.includes('aggressive') ? 'Aggressive' : 'Balanced',
      status: 'Submitted',
