import uuid
from sqlalchemy import Column, String, TIMESTAMP, text, ForeignKey, Integer, Float, Text, JSON
...
class ForecastGrid(Base):
    ...
    user = relationship("User", back_populates="forecast_grids")
...
class WarningPolygon(Base):
    ...
    # In production, this should be Geometry('POLYGON', srid=4326). Using String for SQLite compatibility in tests.
    ...
    user = relationship("User", back_populates="warning_polygons")
...
class TextProduct(Base):
    ...
    user = relationship("User", back_populates="text_products")
