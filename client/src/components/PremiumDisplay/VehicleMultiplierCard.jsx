...
const VehicleMultiplierCard = ({
  vehicleMultiplierAmount,
  vehicleMultiplier,
}) => {
  ...
  return (
    <div className="bg-surface border border-outline-variant rounded-xl p-md flex justify-between items-center shadow-sm">
      <div className="flex items-center gap-md">
        <div
          className={`w-10 h-10 rounded-lg ${iconBg} bg-opacity-20 flex items-center justify-center ${iconText}`}
        >
          <span className="material-symbols-outlined">{icon}</span>
        </div>
        <span className="font-body-md text-body-md text-on-surface">
          Vehicle Multiplier ({vehicleMultiplier?.toFixed(1)}x)
        </span>
      </div>
      <span className={`font-headline-md text-headline-md ${textColor}`}>
        {sign}${Math.abs(vehicleMultiplierAmount)?.toFixed(2)}
      </span>
    </div>
  );
};
