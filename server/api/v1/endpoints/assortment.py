...
@router.post("/scenario-projections", response_model=schemas.ScenarioProjectionsResponse)
...
def submit_assortment_decision(payload: schemas.AssortmentDecisionRequest, db: Session = Depends(get_db)):
    scenario_type = payload.scenario_type
    if scenario_type not in ["Conservative", "Balanced", "Aggressive"]:
        raise HTTPException(status_code=400, detail="Invalid scenario type")
    
    # Find or create scenario in DB
    scenario = db.query(models.AssortmentScenario).filter(models.AssortmentScenario.name == scenario_type).first()
    if not scenario:
        sales_lift = 0.5 if scenario_type == "Conservative" else (2.0 if scenario_type == "Balanced" else 4.5)
        pb_pct = 23.0 if scenario_type == "Conservative" else (25.0 if scenario_type == "Balanced" else 28.0)
